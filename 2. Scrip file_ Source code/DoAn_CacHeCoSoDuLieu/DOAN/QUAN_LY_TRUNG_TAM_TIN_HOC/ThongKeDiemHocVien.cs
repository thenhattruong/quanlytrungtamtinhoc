...
{
    public partial class ThongKeDiemHocVien : Form
    {
        public ThongKeDiemHocVien()
        {
            InitializeComponent();
        }
        ...
        private void ThongKeDiemHocVien_Load(object sender, EventArgs e)
        {
            string queryMaLop = "SELECT MaHV FROM DIEM";
            DataTable resultMaLop = sqlInstance.ExecuteQuery(queryMaLop);

            if (resultMaLop.Rows.Count > 0)
            {
                cbxMaHocVien.DataSource = resultMaLop;
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            try 
            {
                string selectedMaLop = cbxMaHocVien.SelectedValue.ToString(); // Lấy giá trị đã chọn trong combobox

                // Truy vấn dữ liệu dựa trên mã lớp đã chọn
                string query = $"SELECT* FROM TimDiemHocVienTheoMaHocVien('{selectedMaLop}')";
                DataTable result = sqlInstance.ExecuteQuery(query);

                // Đổ dữ liệu vào dataview
                dgvThongKeDiemHocVien.DataSource = result;
            }
            catch (Exception)
            {
        ...
        private void btnInDiem_Click(object sender, EventArgs e)
        {
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ToExcel(dgvThongKeDiemHocVien, saveFileDialog1.FileName);
                }
